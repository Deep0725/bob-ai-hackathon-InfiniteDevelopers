...
function processSignalData(data) {
  ...
  setTimeout(() => {
    try {
      ...
      // ── IBM Granite AI layer (non-blocking — runs after UI is visible) ──
      if (typeof requestSignalAIAssessment === 'function') {
        const detectedSignals = signals.filter(s => s.isSignal);
        if (detectedSignals.length > 0) {
          requestSignalAIAssessment(detectedSignals);
        }
      }
    } catch (err) {
...
function analyzeDossierText(text) {
  ...
  setTimeout(() => {
    ...
    // ── IBM Granite AI layer (non-blocking — runs after UI is visible) ──
    if (typeof requestRegulatoryAIAssessment === 'function') {
      requestRegulatoryAIAssessment(analysis);
    }
  }, 700);
}
